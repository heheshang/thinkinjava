...
public class E13_OverloadedFill {
    ...
    public static void main(String[] args){
        ...
        for ( Coffee c : coffeeList )
            System.out.println("coffeeList="+c);
        Queue<Integer> iQueue = fill(( Queue<Integer> ) new LinkedList<Integer>(), new Fibonacci(), 12);
        for ( int i : iQueue )
            System.out.print("iQueue="+i + " ");
        System.out.println();
        LinkedList<Character> cLList = fill(new LinkedList<Character>(), new CountingGenerator.Character(), 26*2);
        for ( char ch : cLList )
            System.out.print(""+ch);
        System.out.println();
        Set<Boolean> bSet = fill(new HashSet<Boolean>(), new CountingGenerator.Boolean(), 10);
        for ( Boolean b : bSet )
            System.out.println("bSet="+b);
    }
}
